...
* Funkcionalni profil mladih: **KN-FS-002 v1.0.10** — `docs/functional-specifications/Functional-Specification_Konkursi_Mladi.md` (USVOJEN)
...
| 1.0.6 | 2026-09-21 | KN-PATCH-TS-006 — Predložen youth-only konceptualni model izjednačene grupe, kruga/predloga raspodjele, članstva prijava sa nacrtnim iznosima, tri pojedinačna glasa po kanonskom mjestu i zaključavanja kruga; nije migracija ni implementacija. Predmet glasa je kompletan predlog, ne pojedinačna prijava. Živi pokazivači: KN-BM-002 v1.0.12, KN-FS-002 v1.0.10, KN-RG-001 v1.0.39, KN-FS-003 v1.0.17. Ženski tok nije diran. |
...
Svih 14 poglavlja je sadržajno uneseno i ima status `USVOJENO`. Dokument ima status `USVOJEN`. Tabele `commission_sessions` i `commission_session_attendances` već postoje za prvu sjednicu; naredna implementacija usmenog i druge sjednice proširuje taj model. Ostale predložene kolone u Poglavlju 13 **nijesu** već sve implementirane u kodu; usvajanjem ovog dokumenta odobrena je njihova tehnička namjena.
...
| KN-RG-001 | Registar skraćenica i oznaka dokumentacije Konkursa | 1.0.39 | USVOJENO | evidencija Document ID-a ovog dokumenta |
...
| KN-FS-002 | Funkcionalna specifikacija konkursa za podršku preduzetništvu mladih | 1.0.10 | USVOJEN | **SSOT** funkcionalnog ponašanja mladih; 142 prihvatna kriterijuma |
...
Ženski profil **ne** dobija obavezno grupisanje `(type, year)`, redni broj Poziva niti polja `call_number` / `annual_budget`. Ženska forma i dalje prikazuje `Ukupan budžet`.
...
Drugi Poziv postoji samo ako je ispunjen uslov `BM-ML-049`, ako je `canCreateSecondCall` tačno i ako ga Administrator ručno kreira (`BM-ML-051`). `canCreateSecondCall` je kapija mogućnosti, a ne obaveza kreiranja.
...
* `competitions.budget` — ukupan budžet konkretnog Poziva;
* `competitions.annual_budget` — interna kolona godišnjeg okvira, radi godišnje kontrole i drugog Poziva.

Pravila:

* na formi prvog Poziva Administrator unosi samo polje `Ukupan budžet`, kao na ženskoj formi;
* klijent **ne** šalje nezavisnu vrijednost `annual_budget`;
* pri kreiranju prvog omladinskog Poziva server postavlja `annual_budget = budget`;
...
* drugi Poziv nasljeđuje `annual_budget` prvog; korisnik ga ne bira proizvoljno;
...
* ženski tok zadržava postojeće budžetsko ponašanje i **ne** dobija `call_number` ni `annual_budget`;
* ženska forma i dalje prikazuje `Ukupan budžet` i **ne** dobija nova polja na formi.

Primjer: `Ukupan budžet` / `budget` / `annual_budget` prvog Poziva 100.000, potvrđena raspodjela prvog 60.000 → `remaining_after_first` 40.000; Administrator može unijeti `budget` drugog najviše 40.000 i veći od nule, ali nije obavezan; `budget` prvog ostaje 100.000.

Izvor: `KN-BM-002` Poglavlje 15.1; `KN-FS-002` §5.9.2–5.9.3.
...
Objavljeni `budget` prvog Poziva se **ne** prepisuje.

Platforma **ne** kreira automatski drugi Poziv.

Kada je `remaining_after_first > 0`, Platforma ovlašćenom korisniku **prikazuje** da drugi Poziv **nije** obavezan. Administrator procjenjuje postoji li potreba i **može** odobriti i ručno raspisati (`BM-ML-049`). Kreiranje nacrta ostaje ručna akcija Administratora (`BM-ML-051`) i dostupno je tek kada je `canCreateSecondCall` tačno. Nema workflow statusa ni odobrenja drugog organa.

Detalj raspodjele i potvrde iznosa razrađuje se u Poglavlju 11. Ovo poglavlje samo obavezuje da se `remaining_after_first` računa od budžeta prvog Poziva. Nema kolone `remaining_amount` niti nove tabele instance.
...
* na formi prima samo `Ukupan budžet` i server postavlja `annual_budget = budget`;
...
Za drugi Poziv važi isti postojeći postupak zatvaranja kao kod ženskog preduzetništva: `closeCompetition()`. Ta akcija postavlja `status = completed` i zaključava prvi Poziv. Status `completed` ostaje postojeća zajednička tehnička činjenica. Profil mladih ga samo koristi kao kapiju za drugi Poziv.
...
canCreateSecondCall =
  ...
  && secondCallDoesNotExist
```

Koriste se postojeći `closeCompetition()`, status `completed`, `hasChairmanCompletedDecisions()` i `CompetitionAnnualInstance`.

Dok `canCreateSecondCall` nije tačno, akcija kreiranja nacrta drugog Poziva nije dostupna. Ako prvi Poziv nije `completed`, Platforma prikazuje da postupak prvog nije završen. Zatvaranje prvog **ne** kreira drugi Poziv. Administrator ga pokreće posebno i ručno. Ako je `remaining_after_first` 0, drugi Poziv se ne kreira. Ako je `remaining_after_first > 0`, drugi Poziv i dalje **nije** obavezan. Zatvaranje prvog ne mijenja status njegovih prijava, nije brisanje i nije javna objava.

Ne uvodi se:

* novi competition status;
* `ranking_frozen_at`;
* nova tabela;
* novi snapshot;
* dodatno zaključavanje u `EvaluationController`;
* automatsko kreiranje;
* scheduler.

Ženski profil ne dobija obavezni drugi Poziv niti nova polja na formi.

Izvor: `BM-ML-033`; `BM-ML-049`–`BM-ML-053`; F-02.
...
* drugi Poziv prije `canCreateSecondCall`;
...
* novi competition status, `ranking_frozen_at`, novu tabelu, novi snapshot ili dodatno zaključavanje u `EvaluationController`.
...
| Prvi Poziv | stored | zaključivanje / arhiva | preduslovi Poglavlja 12; `closeCompetition()` | prvi Poziv `completed`; prijave se ne brišu; drugi Poziv se ne kreira | predsjednik | F-05; Poglavlje 12 |
| Instanca | izvedeno | obračun `remaining_after_first` | `budget` prvog minus potvrđene raspodjele prvog Poziva | preostali iznos | sistem | `BM-ML-049`; Poglavlje 11 |
| Drugi Poziv | stored | ručno kreiranje nacrta | `canCreateSecondCall`; postoji prvi Poziv istog `type` i `year`; nema trećeg | nacrt sa `call_number = 2` | Administrator | `BM-ML-051`; `BM-ML-050` |
...
**Ako:** nema prvog Poziva instance, ili `canCreateSecondCall` nije tačno.

**Kada:** korisnik pokuša kreirati drugi.

**Onda:** kreiranje je zabranjeno. Ako prvi Poziv nije `completed`, Platforma prikazuje da postupak prvog nije završen.

Izvor: `BM-ML-050`; `BM-ML-051`.
...
### 4.11.7 — Preostala sredstva i opcionost

**Ako:** nakon `closeCompetition()` prvog Poziva je `remaining_after_first > 0`.
...
Ovo poglavlje određuje tehničku realizaciju Komisije od tri mjesta, formalnog kompletiranja, sjednice, kvoruma i zamjene za `omladinsko`. **Ne** uvodi paralelni sistem Komisije. Sjednice koriste postojeće tabele `commission_sessions` i `commission_session_attendances`.
...
Postojeći `Commission` / `CommissionMember` **ne** čuvaju sjednicu konkretnog Poziva, prisutne članove ni trenutak evidencije. Ta evidencija već živi u `commission_sessions` i `commission_session_attendances`, primjenjivo na sve profile. Ne pravi se tabela samo za profil mladih.

Mora se moći dokazati: koja Komisija, tip sjednice, datum i vrijeme, prisutni članovi, važeće ovlašćenje, kvorum i izvršene radnje. Fizički nazivi tabela navedeni su u Poglavlju 13. Tabele **postoje** za prvu sjednicu; usmeno i druga sjednica proširuju isti model. **Ne** uvodi se paralelni sistem sjednica ni automatsko zakazivanje.
...
Ženski tok ostaje nepromijenjen. Profil mladih **parametrizuje** isti tok vrijednostima već usvojenim u `KN-BM-002` v1.0.12 i `KN-FS-002` v1.0.10:
...
* izbor osporenog pri podnošenju čuva se na istom redu kolonama `criterion_1_contested`, `criterion_2_contested`, `criterion_3_contested`, koje **postoje** na `application_prigovors`;
* te kolone su nullable boolean: `true` = aktiviran i osporen; `false` = aktiviran, nije osporen; `NULL` = nije aktiviran ili profil koji ih ne koristi;
* ženski postojeći tok ostavlja contested kolone `NULL`;
...
Prva sjednica već koristi `commission_sessions` i `commission_session_attendances`. Usmeno i druga sjednica su **proširenje tog postojećeg modela**, ne poseban sistem samo za profil mladih. Fizički nazivi ostaju Poglavlju 13.
...
Dozvoljena su dva puta: M3 nije aktivirao nijedan razlog; ili je M3 aktivirao razlog, ali je konačnom odlukom po prigovoru svaki preostali razlog otklonjen. Istorijski M3, uključujući kriterijum 1 `Potpuna` / `Nepotpuna`, ostaje nepromijenjen. Kapija **ne** zahtijeva da kriterijum 1 i dalje bude `Potpuna` nakon uspješnog prigovora.

Predsjednik, za prijave koje ispunjavaju tu kapiju:
...
Platforma **ne** određuje termin umjesto Komisije, **ne** izmišlja novi termin i **ne** zakazuje automatski (`BM-ML-034`).

Izvor: `KN-FS-002` §15.1; `BM-ML-034`; `BM-ML-043`.
...
Rang-lista prelazi u konačnu fazu tek kada predsjednik potvrdi **cijelu listu odjednom**. Prije potvrde: svaka granična izjednačena grupa ima usvojen i zaključan predlog raspodjele; limiti 30/20/15 potvrđeni za svaku podržanu prijavu. Ženski rok od 45 dana **ne** primjenjuje se.
...
Ako sredstva nijesu dovoljna za sve planove sa istom punom ocjenom, pravilo se **aktivira samo** tada, prema `BM-ML-046`, i to istovremeno:

1. najmanje dvije prijave imaju isti puni, nezaokruženi rezultat i dijele rang-poziciju;
2. raspoloživi budžet nije dovoljan za sve prijave iz relevantne izjednačene grupe, mjereno **nacrtnim iznosima** nakon 30%/20%/15%;
3. pravilo prednosti otpočinjanja samo po sebi ne određuje kompletnu raspodjelu.

Prednost otpočinjanja primjenjuje se **prije** glasanja (`business_stage`: `započinjanje` nad `razvoj`). Ako budžet pokriva sve prijave prioritetne kategorije, glasanje nije potrebno. Ako ne pokriva, glasa se samo unutar te kategorije. Razvoj se **ne** bira automatski umjesto prioritetne prijave čiji trenutni nacrtni iznos ne staje u ostatak; predsjednik prvo usklađuje nacrt sa dozvoljenim ostatkom, uz obrazloženje za smanjeni iznos.
...
Predsjednik evidentira sva tri pojedinačna glasa `za`/`protiv` u ime Komisije. Članovi **nemaju** poseban elektronski login tok. Predsjednik **ne** smije zaključiti krug nakon prva dva `za`. Tek poslije sva tri glasa: 3–0 ili 2–1 usvaja predlog; 1–2 ili 0–3 ne usvaja. Manje od tri glasa = glasanje nije završeno.

Jedna grupa može imati više krugova: jedan aktivni nacrt; zaključani krug je nepromjenjiv; neusvojeni ostaje istorijski; usvojeni krug zabranjuje novi. Konačna lista čeka usvojen i zaključan predlog svake granične grupe.

Glas pripada kanonskom mjestu 1–3 i tadašnjem članu. Zamjena prije glasanja daje glas novom aktivnom članu; zamjena poslije zaključavanja ne prepisuje istoriju. Rang-pozicija i bodovi ostaju. ID, vrijeme, abeceda i automatska optimizacija budžeta **nijesu** kriterijumi.

**Ne** koristi se ženski model glasova (5 članova, 3 od 5, ženske kolone, ishod bez pojedinačnih glasova). Youth audit je zaseban, prema §13.9. **Ne** uvodi se obavezni elektronski `TYPE_THIRD`.
...
Izvor: `BM-ML-046`; `BM-ML-044`; `KN-FS-002` §20.4 i §20.7.5.
...
Drugi Poziv **nije obavezan** kada nakon prvog ostanu sredstva (`BM-ML-049`). Ostatak daje mogućnost, a ne obavezu. Administrator procjenjuje postoji li potreba i može odobriti i ručno raspisati drugi Poziv. Administrator nije obavezan da ga raspisuje. Platforma ga **ne** kreira ni objavljuje automatski. Nema workflow statusa ni odobrenja drugog organa.

Administrator ručno kreira novi zapis u `competitions`, u **istoj** godišnjoj instanci, sa sopstvenim budžetom, zavodnim brojem, objavom i rokom, tek kada je `canCreateSecondCall` tačno:
...
Koriste se postojeći `closeCompetition()`, status `completed`, `hasChairmanCompletedDecisions()` i `CompetitionAnnualInstance`. Status `completed` ostaje postojeća zajednička tehnička činjenica. Ženski profil ne dobija obavezni drugi Poziv niti nova polja na formi.
...
| Izjednačenje | prednost otpočinjanja dovoljna | budžet pokriva prioritetnu kategoriju | bez glasanja | `BM-ML-046` |
| Izjednačenje | kompletan predlog raspodjele | predsjednik; nacrti nakon 30/20/15 | predlog spreman; Platforma ne bira | `BM-ML-046` |
| Izjednačenje | evidencija tri glasa | kanonska mjesta 1–3; `za`/`protiv` | sva tri glasa; krug nije zaključen dok treći nije upisan | `BM-ML-046` |
| Izjednačenje | zaključavanje kruga | 3–0 ili 2–1 | predlog usvojen; krug nepromjenjiv | `BM-ML-046` |
| Izjednačenje | zaključavanje kruga | 1–2 ili 0–3 | predlog nije usvojen; dozvoljen novi krug | `BM-ML-046` |
| Rang-lista | potvrda cijele liste | svaka granična grupa ima usvojen i zaključan predlog; iznosi valjani | konačna faza; `approved` / `rejected` | `BM-ML-044`; `BM-ML-045` |
...
| Drugi Poziv | ručno kreiranje | `canCreateSecondCall` | nacrt poziva 2; ista instanca; prvi ostaje `completed` | `BM-ML-051` |
...
**Onda:** dijele poziciju po obrascu `1, 2, 2, 4`. ID, vrijeme i abeceda ne daju prednost. Ako budžet nije dovoljan i prednost otpočinjanja nije dovoljna, predmet glasa je kompletan predlog raspodjele grupe; sva tri glasa `za`/`protiv` su obavezna; većina 2 od 3 tek poslije trećeg glasa.
...
**Onda:** Administrator ga kreira ručno u istoj instanci tek kada je `canCreateSecondCall` tačno. Platforma ga ne kreira. Treći Poziv je zabranjen. Nova prijava je `draft` bez prenosa. Ženski tok ostaje bez obaveznog drugog Poziva i bez novih polja na formi.
...
Koristi se postojeća akcija zatvaranja ženskog konkursa kao tehnička osnova: `closeCompetition()`.

Ta akcija postavlja `status = completed` i `closed_at`. Status `completed` ostaje postojeća zajednička tehnička činjenica. Profil mladih ga samo koristi kao kapiju za drugi Poziv. Ne uvodi se novi competition status, `ranking_frozen_at`, nova tabela, novi snapshot, dodatno zaključavanje u `EvaluationController`, automatsko kreiranje ni scheduler.
...
Arhiviranje prvog Poziva **ne** kreira drugi Poziv. Preostala sredstva ostaju evidentirana. `closeCompetition()` postavlja `completed` i zaključava prvi Poziv. To je tehnička kapija V1 poslije koje Administrator može ručno kreirati drugi Poziv prema Poglavlju 4 i §11.9, ako je `canCreateSecondCall` tačno. Zatvaranje prvog ne mijenja status njegovih prijava, nije brisanje i nije javna objava.
...
| Drugi Poziv | arhiviranje prvog | ostala sredstva; `closeCompetition()` | prvi `completed`; preostali iznos evidentiran; drugi se ne kreira | `BM-ML-051` |
...
Izvori: `KN-BM-002` v1.0.12; `KN-FS-002` v1.0.10; ovaj dokument Poglavlja 2–12; postojeći ženski kod **samo kao tehnički obrazac**.
...
* drugi Poziv zahtijeva postojeći prvi iste instance: isti `type` i `year`, `call_number = 1`, te `canCreateSecondCall`;
* nema trećeg;
...
* ne uvodi se novi competition status, `ranking_frozen_at`, nova tabela, novi snapshot ni dodatno zaključavanje u `EvaluationController`;
* ženski tok ostaje bez `call_number`, `annual_budget` i bez novih polja na formi; ženska forma i dalje prikazuje `Ukupan budžet`.
...
Za usmeno obrazloženje potrebna su **sva tri aktivna člana**. Ocjenjivanje profila mladih ostaje zatvoreno dok usmeno konkretne prijave nije evidentirano kao završeno (`BM-ML-039`).
...
Izvor: `BM-ML-001`–`BM-ML-003`, `BM-ML-007`, `BM-ML-034`, `BM-ML-039`; ovaj dokument Poglavlja 8 i 10.
...
| Izbor osporenih pri podnošenju | `application_prigovors.criterion_1\|2\|3_contested` | da — kolone postoje | nema nove tabele | `true` = aktiviran i osporen; `false` = aktiviran, nije osporen; `NULL` = nije aktiviran ili `zensko`; prigovor i M3 ostaju odvojeni zapisi |
...
Kolone `criterion_1_contested`, `criterion_2_contested` i `criterion_3_contested` **postoje** na `application_prigovors`. Nullable su. `true` = aktiviran i osporen; `false` = aktiviran, nije osporen; `NULL` = nije aktiviran ili profil koji ih ne koristi. Ženski postojeći tok ostavlja ih `NULL`. Prigovor i M3 ostaju odvojeni zapisi. Nova tabela se **ne** uvodi.
...
| Glasovi izjednačenja 2 od 3 | ženske tabele/kolone glasova | **ne** | predloženi youth-only konceptualni model ispod | ne ženski 5 članova / 3 od 5; ne pojedinačni glas po prijavi |
...
Predloženi konceptualni model glasanja izjednačenja (**nije migracija ni implementacija ovog PATCH-a**). Imena ispod su logička, u postojećem youth namespace-u; **nijesu** runtime ključevi.

1. **Izjednačena grupa** (npr. `youth_equal_score_groups`): Poziv, puna ocjena, zajednička rang-pozicija, skup prijava.
2. **Krug / predlog raspodjele** (npr. `youth_equal_score_rounds`): jedan kompletan predlog za cijelu grupu; raspoloživi budžet prije predloga; ukupan iznos predloga; obrazloženje; primijenjeno pravilo prednosti otpočinjanja; status nacrta ili zaključanog kruga; ishod usvojen / nije usvojen / nije završen.
3. **Članstvo prijava u predlogu** (npr. `youth_equal_score_round_applications`): svaka prijava grupe; poslovna faza; podržana ili nije; tačan nacrtni iznos ako je podržana; primijenjeni youth limit 30%/20%/15%.
4. **Tri pojedinačna glasa** (npr. `youth_equal_score_votes`): kanonsko mjesto 1, 2 ili 3; tadašnji `commission_member_id`; `user_id`; ime člana kao snapshot; `za` ili `protiv`; datum i vrijeme; predsjednik koji je glas evidentirao (`recorded_by_user_id`).
5. **Zaključavanje i rezultat kruga:** poslije sva tri glasa predsjednik zaključava krug; zaključani krug je nepromjenjiv; 3–0 ili 2–1 usvaja; 1–2 ili 0–3 ne usvaja.
6. **Audit predsjednika:** ko je evidentirao glasove i ko je zaključao krug.

Ograničenja konceptualnog modela:

* jedan aktivni nacrt kruga po grupi;
* jedan glas po kanonskom mjestu u krugu;
* tačno mjesta 1–3;
* zaključan krug je nepromjenjiv;
* najviše jedan usvojeni krug po grupi.

Ravna tabela po `application_id` **nije** dovoljna: predmet glasa je predlog raspodjele cijele grupe, ne pojedinačna prijava. Ženske kolone i ženske tabele glasova se **ne** koriste. Članovi **nemaju** poseban login tok.
...
| 3 | `competitions.call_number`, `competitions.annual_budget` | redni broj 1\|2; interni godišnji okvir jednak `budget`-u prvog Poziva | nullable; ženski redovi NULL | unique NULL-siguran za `zensko` | drop kolona; unique drop |
...
| 5 | postojeće `commission_sessions` + `commission_session_attendances` | proširenje postojećeg modela za drugu sjednicu i usmeno; **nema** paralelne tabele sjednica | prva sjednica već postoji; ženski tok nepromijenjen | srednji (proširenje unique/kolona) | ne drop postojećih tabela prve sjednice |
| 6 | postojeće `application_prigovors.criterion_1_contested`, `criterion_2_contested`, `criterion_3_contested` | izbor osporenog pri podnošenju; nije zamjena za `criterion_*_remaining` | kolone već postoje; ženski redovi `NULL` | nisko | ne drop postojećih kolona |
...
flowchart TD
  ...
  sessions["commission_sessions postoji; prva sjednica"]
  attendances["commission_session_attendances postoji"]
...
| Komisija | §8, §13.7 | `BM-ML-001`–`007` | §3.2 | kvorum 2; tri člana; postojeće tabele prve sjednice; usmeno/druga sjednica proširuju model |
...
* skala 1–5;
* usmeno i druga sjednica samo uz kapiju §10.2 / FS 15.7.1;
* sva tri aktivna člana za usmeno;
* nema automatskog zakazivanja;
* `Završi ocjenjivanje` blokiran dok usmeno konkretne prijave nije završeno;
...
**Kraj dokumenta KN-TS-002 v1.0.6**
