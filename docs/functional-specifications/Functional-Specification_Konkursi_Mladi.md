...
`KN-FS-003` v1.0.17 služi samo kao strukturni obrazac i uporedni izvor. Zajednički funkcionalni obrazac smije se preuzeti samo kada **nije** suprotan `KN-BM-002`.
...
Poslovni rokovi sjednica (`BM-ML-034`) evidentiraju se kao sljedeće činjenice. Platforma **ne** zakazuje sjednice automatski. Prva i druga sjednica već imaju elektronsku evidenciju prisustva. Treća sjednica **nije** obavezni elektronski `TYPE_THIRD`: poslovni događaj može biti van Platforme; Platforma evidentira zaključke, glasove i konačnu potvrdu.
...
| Sjednica Komisije | druga sjednica i usmena održani | treća sjednica | najkasnije sedam dana od druge sjednice i usmenih intervjua; nije obavezni `TYPE_THIRD`; događaj može biti van Platforme | zaključci, glasovi i konačna potvrda evidentirani; sva tri aktivna člana za punovažnu odluku | Komisija | `BM-ML-034`; `BM-ML-003`; `KN-PATCH-FS-014` |
...
| Poziv | konačni rezultat; iznosi evidentirani | ručno zaključi i arhiviraj | svi preduslovi iz §23.2 ispunjeni | Poziv arhiviran; status prijave nepromijenjen; nije obrisan; drugi Poziv se ne kreira automatski; zatvaranje je kapija poslije koje se drugi Poziv može ručno kreirati | predsjednik / sistem blokira ako uslovi nisu ispunjeni | F-05; `BM-ML-058`; `BM-ML-051` |
...
**Onda:** poslovni rok je najkasnije sedam dana od održavanja druge sjednice i usmenih intervjua. Platforma **ne** zakazuje treću sjednicu automatski. **Ne** uvodi se obavezni elektronski `TYPE_THIRD`. Poslovni događaj može biti van Platforme. Platforma evidentira zaključke, glasove i konačnu potvrdu. Za punovažnu youth odluku učestvuju sva tri aktivna člana.

Izvor: `BM-ML-034`; `BM-ML-003`; `KN-PATCH-FS-014`.
...
**Ako:** budžet drugog Poziva nije veći od nule, ili bi premašio preostala godišnja sredstva nakon prvog Poziva, ili bi ukupna konačno potvrđena raspodjela oba Poziva premašila odobreni godišnji budžet.
...
**Onda:** dodjeljuje se najviše **3** boda, ne 6. Platforma **ne** odlučuje automatski o inovativnosti ni zelenosti. Ovaj +3 bod **nije** dokaz ni izvor limita od 30% iz člana 19 / `BM-ML-047`.
...
Platforma **ne** dodaje eliminatorni razlog koji nije propisan Odlukom. M3 kriterijum 2 **nije** evidencija ranijeg youth finansiranja za limite 20% i 15% iz `BM-ML-047`. Ta činjenica vodi se posebno, prema §21.2.
...
Konačna faza rang-liste poslovno se veže za treću sjednicu. Komisija treću sjednicu zakazuje najkasnije sedam dana od održavanja druge sjednice i usmenih intervjua (`BM-ML-034`). Platforma **ne** zakazuje sjednice automatski. **Ne** uvodi se obavezni elektronski `TYPE_THIRD`. Poslovni događaj može biti van Platforme; Platforma evidentira zaključke, glasove i konačnu potvrdu. Nacrt raspodjele može biti pripremljen ranije, ali **nema** konačno dejstvo.

Prenosi se samo procesni princip ženskog toka (događaj može biti van Platforme; Platforma čuva zaključke). **Ne** prenose se ženski procenti, broj članova Komisije ni ženski način evidencije glasanja.
...
Predsjednik **potvrđuje cijelu listu odjednom**. Nacrt raspodjele **nema** konačno dejstvo. Ženski rok od 45 dana **ne** primjenjuje se.

Nakon potvrde:

* bodovi se ne mijenjaju;
* redosljed se ne mijenja proizvoljno;
* iznosi, nacrt i činjenice se zaključavaju;
...
* tek tada `remaining_after_first` računa odobrenu raspodjelu.
...
**Onda:** bodovi, redosljed, iznosi, nacrt i činjenice se zaključavaju. Podržane prijave sa iznosom postaju `approved`. Prijave ispod 30 i prijave bez sredstava postaju `rejected` sa tipiziranim razlogom. Tek tada `remaining_after_first` računa odobrenu raspodjelu. Ženski rok od 45 dana **ne** primjenjuje se. Javna objava Odluke se **ne** pokreće.

Izvor: `BM-ML-045`; `BM-ML-048`.
...
* **30%** — samo **potvrđeni inovativni tehnološki start-up** u smislu člana 19;
* **20%** — ako podnosilac ranije **nije** dobio sredstva Opštine Kotor za podršku preduzetništvu mladih;
* **15%** — ako **jeste** ranije dobio takva sredstva.

Ranije finansirano fizičko lice pripada limitu **15%**.

Primjenjuje se **najveći odgovarajući** procenat. Procenti se **ne** sabiraju.

Osnovica je budžet konkretnog Poziva. Iznos je dodatno ograničen traženim iznosom i preostalim budžetom Poziva. Procenat je **maksimum**, ne automatski iznos.

Status inovativnog tehnološkog start-upa i ranije youth finansiranje vode se u **posebnim evidencijama**. **Ne** izvode se iz `business_stage`, dodatnih +3 boda (`bonus_green_innovative`) ni M3 kriterijuma 2 (`criterion_2`). Činjenice potvrđuje ovlašćeni administratorski ili komisijski tok i zaključavaju se uz konačnu listu. Tehnička specifikacija predlaže minimalna audit polja **bez** vezivanja za ženske kolone.
...
Nakon **konačne** potvrde raspodjele prvog Poziva Platforma:

* evidentira ukupno raspodijeljeni iznos prijava u statusu `approved`;
* računa i prikazuje `remaining_after_first`.

Nacrtni iznosi **ne** ulaze u taj izvod. Predsjednik potvrđuje raspodjelu **cijele liste odjednom**. Postojanje preostalih sredstava daje mogućnost, a ne obavezu, za drugi Poziv. Platforma **ne** kreira i **ne** objavljuje drugi Poziv automatski. Detalj: Poglavlje 22.
...
| Objekat | Početna činjenica | Akcija/događaj | Uslov | Rezultat | Uloga | BM/F izvor |
|---------|-------------------|----------------|-------|----------|-------|------------|
| Limit | rangirana prijava | izračun limita | budžet konkretnog Poziva; posebne evidencije start-up i ranijeg finansiranja | 30/20/15%; najveći; ne sabiraju se; nije iz `business_stage`, +3 ni M3 | sistem | `BM-ML-047` |
...
**Onda:** procenti se ne sabiraju. Primjenjuje se najveći odgovarajući procenat. 30% pripada samo potvrđenom inovativnom tehnološkom start-upu iz posebne evidencije. 20% ili 15% pripadaju posebnoj evidenciji ranijeg youth finansiranja. Te činjenice se ne izvode iz `business_stage`, +3 ni M3. Ranije finansirano fizičko lice koristi 15% ako nema 30%. Ženski limiti 20/10/5% **nijesu** dostupni.
...
**Onda:** iznosi, nacrt i činjenice se zaključavaju uz lice, datum i vrijeme. Podržane prijave sa iznosom postaju `approved`. Prijave ispod 30 i prijave bez sredstava postaju `rejected` sa tipiziranim razlogom. Tek tada `remaining_after_first` računa odobrenu raspodjelu. Drugi Poziv se **ne** kreira automatski. Javni tok akata se **ne** pokreće. Ženski rok od 45 dana **ne** primjenjuje se.
...
Dugme ili akcija za drugi Poziv **nije** dostupna dok prvi Poziv nema status `completed`. Prije toga Platforma prikazuje da postupak prvog Poziva nije završen.

U V1 su `hasChairmanCompletedDecisions()` i status `completed` tehnička kapija. Koristi se isti postojeći postupak zatvaranja kao kod ženskog preduzetništva. Predsjednik završava sve odluke i iznose. Tek nakon te kapije, i samo ako je `remaining_after_first` veći od nule, Administrator može ručno kreirati nacrt drugog Poziva.

Ako su sva godišnja sredstva raspodijeljena, odnosno ako ostatak nije veći od nule:
...
Godišnja instanca može imati samo prvi Poziv (`call_number` 1) i eventualno drugi Poziv (`call_number` 2). Pokušaj kreiranja trećeg Poziva je **blokiran**.

Podaci prvog Poziva ostaju **nepromijenjeni i sačuvani**. Arhivirani ili zatvoreni prvi Poziv ostaje nepromijenjen. Drugi Poziv je novi odvojeni zapis. Platforma čuva vezu između godišnje instance, prvog Poziva i drugog Poziva.

Ovo poglavlje **ne** određuje tehnički model baze ni konkretne ID strukture.

## 22.3. Ručno kreiranje i objavljivanje

Administrator Konkursa **ručno** kreira drugi Poziv tek nakon što je prvi Poziv `completed`, sve odluke predsjednika su završene i `remaining_after_first` je veći od nule. Prije te kapije akcija nije dostupna. Zatvaranje prvog Poziva **ne** kreira drugi Poziv.
...
| Treći Poziv | postoje prvi i drugi Poziv iste instance | pokušaj kreiranja | bilo koji | blokirano | sistem | `BM-ML-050` |
| Drugi Poziv | nacrt | čuvanje | — | sačuvan; nije objavljen; rok ne kreće | Administrator | `BM-ML-051`; `BM-ML-033` |
| Budžet drugog Poziva | unos | potvrda | iznos nije veći od nule ili premašuje preostala godišnja sredstva nakon prvog Poziva | blokirano; razlog prikazan | sistem | `BM-ML-049`; `BM-ML-047` |
| Budžet drugog Poziva | unos | potvrda | 0 < iznos ≤ preostala godišnja sredstva nakon prvog; zbir potvrđenih raspodjela ne premašuje godišnji budžet | budžet sačuvan; nije automatski upisan; objavljeni budžet prvog nepromijenjen | Administrator | `BM-ML-049`; Poglavlje 5 |
...
**Ako:** su nakon potvrđene raspodjele prvog Poziva preostala godišnja sredstva jednaka nuli ili nijesu veća od nule.

**Kada:** Platforma prikazuje stanje godišnje instance ili Administrator pokuša kreirati drugi Poziv.

**Onda:** drugi Poziv se ne raspisuje i ne kreira. Platforma ne nudi automatsko kreiranje drugog Poziva.
...
**Onda:** Platforma ne kreira i ne objavljuje drugi Poziv automatski. Zatvaranje prvog Poziva samo po sebi ne kreira drugi Poziv.

Izvor: `BM-ML-051`.

### 22.7.4 — Ista godišnja instanca

**Ako:** Administrator ručno kreira drugi Poziv ili pokuša kreirati treći Poziv.

**Kada:** se uspostavlja veza sa godišnjom instancom.

**Onda:** drugi Poziv pripada istoj godišnjoj instanci kao prvi. Godišnja instanca može imati samo `call_number` 1 i eventualno `call_number` 2. Pokušaj kreiranja trećeg Poziva je blokiran. Nova godišnja instanca se ne kreira samo zbog drugog Poziva. Podaci prvog Poziva ostaju nepromijenjeni i sačuvani. Arhivirani ili zatvoreni prvi Poziv ostaje nepromijenjen. Drugi Poziv je novi odvojeni zapis. Ručno kreiranje nacrta dozvoljeno je tek kada je prvi Poziv `completed`, `hasChairmanCompletedDecisions()` tačno, `remaining_after_first > 0` i drugi Poziv još ne postoji.
...
**Ako:** uneseni budžet drugog Poziva nije veći od nule, ili premašuje preostala godišnja sredstva nakon prvog Poziva, ili bi ukupna konačno potvrđena raspodjela oba Poziva premašila odobreni godišnji budžet.

**Kada:** se pokuša potvrditi budžet.

**Onda:** Platforma blokira potvrdu i prikazuje razlog razumljiv Administratoru. Nula, negativan iznos i iznos iznad preostalih godišnjih sredstava nijesu dozvoljeni. Budžet se ne postavlja automatski. Objavljeni budžet prvog Poziva ostaje nepromijenjen.
...
U V1 postojeće zatvaranje prvog Poziva postavlja status `completed` i zaključava prvi Poziv. To je tehnička kapija poslije koje se može kreirati drugi Poziv. Drugi Poziv se **ne** kreira samom akcijom zatvaranja. Administrator ga pokreće posebno i ručno prema Poglavlju 22, tek kada su sve odluke predsjednika završene, prvi Poziv je `completed` i `remaining_after_first > 0`. Zatvaranje prvog ne mijenja status njegovih prijava. Dokumentacija i podaci prvog ostaju sačuvani. Zatvaranje prvog nije brisanje niti javna objava.
...
| Poziv | arhiviranje sa preostalim sredstvima | potvrda predsjednika | preduslovi ispunjeni | Poziv arhiviran; status `completed`; preostali iznos ostaje evidentiran; drugi Poziv se ne kreira; zatvaranje nije brisanje ni javna objava | predsjednik / sistem | F-05; `BM-ML-049`; `BM-ML-051` |
| Drugi Poziv | prvi Poziv arhiviran / `completed`; preostala sredstva | pokušaj automatskog kreiranja | — | zabranjeno; Administrator kreira ručno prema Poglavlju 22 tek nakon kapije; prvi Poziv ostaje arhiviran i nepromijenjen | sistem | `BM-ML-051`; `BM-ML-050` |
...
**Onda:** arhiviranje ne zavisi od ugovora, isplate, realizacije, M4/M4a, faktura, de minimis dokumentacije, praćenja ugovornih obaveza ni javnog objavljivanja službenih akata. Javni PDF tok Odluke se ne pokreće. Postojeće zatvaranje postavlja `completed` i zaključava prvi Poziv. To je tehnička kapija V1 poslije koje se drugi Poziv može ručno kreirati. Arhiviranje prvog Poziva ne kreira drugi Poziv. Preostala sredstva ostaju evidentirana. Zatvaranje prvog ne mijenja status njegovih prijava, nije brisanje i nije javna objava. Prvi Poziv se ne otključava zbog drugog Poziva. Drugi Poziv može nastati tek nakon što je prvi Poziv zatvoren postojećom akcijom `closeCompetition()` i dobije status `completed`; prije ispunjenja te kapije kreiranje drugog Poziva nije dozvoljeno. `closeCompetition()` ne kreira drugi Poziv automatski. Administrator drugi Poziv pokreće posebno i ručno, tek kada su sve odluke predsjednika završene, `remaining_after_first` je veći od nule i drugi Poziv još ne postoji. Treći Poziv nije dozvoljen.
...
* kreirati drugi Poziv prije nego što je prvi `completed`;
* kreirati treći Poziv u istoj godišnjoj instanci;
* drugi Poziv tretirati kao novu godišnju instancu;
* dozvoliti budžet drugog Poziva koji nije veći od nule ili koji premašuje preostala godišnja sredstva nakon prvog Poziva;
* dozvoliti da ukupna konačno potvrđena raspodjela oba Poziva premaši godišnji budžet;
...
| 5.9.3 | Instanca | Budžet drugog Poziva nije veći od nule ili bi premašio preostala godišnja sredstva nakon prvog ili zbir potvrđenih raspodjela godišnji iznos | Potvrda blokirana; razlog prikazan; objavljeni budžet prvog nepromijenjen | 5 | Poglavlje 5 |
...
| 21.8.6 | Raspodjela | Valjana potvrda | Cijela lista; iznosi zaključani; `remaining_after_first` od `approved`; drugi Poziv se ne kreira automatski | 21 | `BM-ML-048`; `BM-ML-051` |
...
| 22.7.6 | Budžet | Iznos nije veći od nule ili je iznad preostalih godišnjih sredstava nakon prvog | Blokirano; razlog prikazan | 22 | `BM-ML-049` |
...
| 23.8.10 | V1 | Arhiviranje | Ne zavisi od ugovora/M4; kapija `completed`; ne kreira drugi Poziv | 23 | F-05; `KN-PATCH-BM-010` |
...
**Onda:** osnovica je budžet konkretnog Poziva. Limiti su 30/20/15. Primjenjuje se najveći odgovarajući procenat. Procenti se ne sabiraju. 30% samo za potvrđeni inovativni tehnološki start-up iz posebne evidencije. 20%/15% iz posebne evidencije ranijeg youth finansiranja, ne iz `business_stage`, +3 ni M3.
...
**Onda:** drugi Poziv se ne kreira automatski. Nacrt se ručno kreira tek nakon `completed` prvog Poziva, završenih odluka predsjednika i `remaining_after_first > 0`.
...
| BM-ML-034 | Rokovi sjednica Komisije | 10.3 | §4.10; §13.2; §14.3; §15.1; §20 | 4.11.6; 4.11.7; 14.6.9; 15.7.1 | Platforma evidentira zaključke; ne zakazuje umjesto Komisije; kapija druge sjednice/usmenog nije istorijska M3 `Potpuna`; treća sjednica nije obavezni `TYPE_THIRD`; nema automatskog produženja roka druge sjednice | U V1 evidencija |
