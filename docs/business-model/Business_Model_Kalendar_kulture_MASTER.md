...
| BM-ORG-20 | **Uklanjanje odbijenog zahtjeva iz uredničkog prikaza (PO-ORG/MOD editor cleanup):** Urednik (`kk_admin`) može ukloniti zahtjev za kreiranje Organizatora iz redovne radne liste **samo** kada je status **Odbijen**. Uklanjanje iz prikaza **nije** brisanje poslovnog zapisa: status ostaje Odbijen; decision metadata (Urednik, vrijeme, napomena) ostaje neizmijenjena; User / Organizer / Moderator grant se ne brišu i ne mijenjaju; novi zahtjev ostaje dozvoljen po BM-ORG-11. Sistem trajno čuva zapis i bilježi ko/kad je uklonio iz prikaza. Odbijanje (poslovna odluka) i uklanjanje iz prikaza (workspace cleanup) su različite radnje. |
...
| BM-MOD-27 | **Uklanjanje odbijenog zahtjeva za Moderatora iz uredničkog prikaza (PO-ORG/MOD editor cleanup):** Urednik (`kk_admin`) može ukloniti zahtjev za ADD ili REMOVE Moderatora iz redovne radne liste **samo** kada je status **Odbijen**. Uklanjanje iz prikaza **nije** brisanje: status ostaje Odbijen; decision metadata KEEP; aktivni Moderator grant ostaje netaknut; resubmission / duplicate pravila (BM-MOD-20) ne mijenjaju se. Sistem trajno čuva zapis i bilježi ko/kad je uklonio iz prikaza. Isto pravilo za rejected ADD i rejected REMOVE. |
