...
> **Napomena (integracija):** Dokument sačuvan iz remote zajedničkog/framework linije kao **KN-BM-004**. Nije trenutni kanonski poslovni SSOT za KN; lokalni SSOT ostaje **KN-BM-001** u `Business_Model_Konkursi.md`.
...
|-----------------|--------|------|
| 0.1.0 | 2026-08-19 | Uspostavljena početna struktura KN-BM-004 kao zajedničkog poslovnog modela modula Konkursi. Poslovni sadržaj poglavlja nije unesen. |
...
| 0.1.19 | 2026-08-25 | Uklonjena planirana Poglavlja 19–21; Poglavlje 18 utvrđeno kao završno sadržajno poglavlje KN-BM-004. |
...
1. KN-BM-004 je izvor istine samo za zajednički apstraktni model modula Konkursi.
...
`KN-BM-004` je zajednički poslovni model modula Konkursi na platformi Digital Kotor. Dokument uspostavlja zajednički okvir za dokumentovanje različitih tipova konkursa, uz obavezno razdvajanje zajedničkog modela od poslovnih profila pojedinačnih tipova konkursa.
...
`KN-BM-004` nalazi se poslije registra `KN-RG-001` u dokumentacionom lancu i predstavlja osnovu za izradu zasebnih poslovnih profila, funkcionalnih specifikacija i zajedničke tehničke specifikacije modula.
...
Svrha dokumenta `KN-BM-004` je da uspostavi zajednički poslovni okvir na kojem se zasnivaju različiti tipovi konkursa u modulu Konkursi.
...
Dokumentacioni opseg `KN-BM-004` obuhvata zajednički poslovni sloj modula Konkursi i njegovu primjenu kroz zasebne poslovne profile različitih tipova konkursa.
...
## KN-BM-004 — zajednički poslovni model

`KN-BM-004` definiše zajednički apstraktni poslovni model modula Konkursi.
...
Ako implementacija odstupa od usvojenog Business Modela, primjenjuje se postojeće pravilo upravljanja `KN-BM-004`: usklađuje se implementacija, osim ako se posebnom poslovnom odlukom ne izmijeni Business Model.
...
`KN-BM-004` proširuje se samo kada novi ili postojeći poslovni profil otkrije poslovni pojam, odnos ili sposobnost koja pripada zajedničkom okviru, a koju postojeći zajednički model još ne podržava.
...
Ono što je zaista zajedničko različitim tipovima pripada zajedničkom poslovnom modelu `KN-BM-004`.
...
Zajednički `KN-BM-004` ne određuje univerzalni katalog razloga za ponovljeni Poziv.
...
`KN-BM-004` definiše zajedničke poslovne aktere i njihove osnovne granice.
...
Zajednički model ne smije nepotrebno razlagati poslovne odgovornosti na dodatne apstraktne aktere. Ako konkretan profil zahtijeva posebnost koja nije pokrivena zajedničkim modelom, ona se prvo poslovno razmatra; ne uvodi se automatski u `KN-BM-004`.
...
`KN-BM-004` ne određuje koje konkretne kategorije lica ili subjekata mogu biti Podnosioci.
...
`KN-BM-004` ne propisuje univerzalno da Komisija uvijek ocjenjuje, boduje, rangira ili donosi konačnu odluku.
...
Ovo poglavlje definiše poslovno značenje apstraktne kategorije, da kategorije nijesu obavezne za svaki tip konkursa, da `KN-BM-004` ne definiše univerzalni katalog konkretnih kategorija, da konkretni poslovni profil određuje koje kategorije koristi i njihova pravila, te granicu prema godišnjoj instanci, Pozivu i implementaciji.
...
`KN-BM-004` ne definiše univerzalni katalog konkretnih kategorija koji važi za sve tipove konkursa.
...
Međutim, `KN-BM-004` ne uvodi unaprijed konkretne vrste kategorizacije niti unaprijed definisane konkretne kategorije.
...
Zajednički `KN-BM-004` ne određuje konkretne Faze pojedinačnih tipova konkursa.
...
`KN-BM-004` ne definiše univerzalni katalog Faza ili Statusa koji važi za sve tipove konkursa.
...
`KN-BM-004` ne definiše univerzalnu state-machine šemu za sve konkurse.
...
Zajednički `KN-BM-004` ne određuje konkretne Obrasce pojedinačnih tipova konkursa.
...
Zajednički `KN-BM-004` ne određuje konkretni katalog Dokumenta za sve konkurse.
...
`KN-BM-004` ne definiše univerzalni katalog kriterijuma.
...
`KN-BM-004` ne definiše univerzalnu skalu bodova i ne pretpostavlja konkretnu skalu.
...
`KN-BM-004` ne definiše univerzalni algoritam rangiranja.
...
`KN-BM-004` ne kreira zajednički katalog eliminacionih uslova.
...
`KN-BM-004` ne propisuje univerzalni sadržaj Prijave.
...
`KN-BM-004` ne propisuje isti režim izmjene i dopune za svaki konkurs.
...
`KN-BM-004` ne propisuje univerzalni konkretni rok niti univerzalnu posljedicu kašnjenja.
...
`KN-BM-004` ne propisuje univerzalno da se nepotpuna Prijava odbija.
...
`KN-BM-004` ne propisuje univerzalno do kojeg trenutka je povlačenje dozvoljeno. To određuje konkretni poslovni profil.
...
`KN-BM-004` ne propisuje da svaki konkurs mora imati Prijedlog odluke.
...
`KN-BM-004` ne propisuje univerzalne ishode Odluke.
...
`KN-BM-004` ne propisuje univerzalni tekst obrazloženja, univerzalnu strukturu, univerzalni katalog razloga odbijanja niti univerzalno ko piše obrazloženje.
...
`KN-BM-004` ne propisuje da svaki konkurs mora imati pojedinačna Rješenja i ne proglašava Rješenje obaveznom univerzalnom sposobnošću svih konkursa.
...
`KN-BM-004` ne propisuje univerzalno sadržaj Ugovora, rok za potpisivanje, način potpisivanja niti posljedice nepotpisivanja.
...
`KN-BM-004` ne propisuje univerzalne vrste izvještaja.
...
`KN-BM-004` ne propisuje da se obavezno arhivira svaki Tip konkursa, godišnja instanca, Poziv, Prijava, Odluka ili drugi pojedinačni poslovni element.
...
`KN-BM-004` ne uvodi univerzalno pravilo da se ništa nikada ne smije obrisati.
...
`KN-BM-004` ne propisuje univerzalnu listu elemenata koji moraju imati sopstveno arhiviranje.
...
`KN-BM-004` ne propisuje univerzalni trenutak arhiviranja.
...
Arhiviranje kao poslovna sposobnost ne znači da `KN-BM-004` uvodi univerzalni poslovni Status `Arhiviran`.
...
`KN-BM-004` ne propisuje univerzalnu zabranu svake naknadne poslovne radnje.
...
`KN-BM-004` to ne propisuje univerzalno svim profilima.

Javna dostupnost istorijskih podataka takođe se ne određuje samim arhiviranjem.

Način prikaza, poseban ekran, filteri, pretraga i slična UI pitanja nijesu predmet `KN-BM-004`.
...
**Kraj dokumenta KN-BM-004 v1.0.1**
