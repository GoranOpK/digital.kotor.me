...
FT-001 je aktivan modul sa završenom i produkcijski potvrđenom **Fazom 6A — Javni portal Događaja** (implementirano i testirano). **6A residual Package A** (`cultural-calendar.day` canonical cutover): **IMPLEMENTED / TESTED / COMMITTED / PUSHED / DEPLOYED / PRODUCTION VERIFIED — EMPTY-DATE SCENARIO CONFIRMED** (`f35cb2e`); content-bearing day scenario not separately production-smoked (covered locally; not a blocker). **Package A CLOSED.** **Phase B** (legacy hard-remove / flag cleanup) ostaje **OPEN**. **Faza 6B — Manifestacije:** **FORMALLY CLOSED** — **ADOPTED / DOCUMENTED / IMPLEMENTED / TESTED / DEPLOYED / PRODUCTION ACCEPTED** (WITH LIMITED CONTENT-SMOKE COVERAGE). PO-MF-WF / PO-EV-WF corrective: **PRODUCTION VERIFIED** (`d3c7a96`).
...
- **PO-TS9-08I:** Privremeni feature flag `legacy` XOR `canonical`; zatim uklanjanje flag-a i legacy public/CRUD.
  - **PHASE 6A-CLOSE-02 (implementirano):** legacy admin CRUD surface `cultural-events.*` (`/kalendar-kulture/dogadjaji`) je **disabled** dedicated middleware-om (`legacy_cultural_events_disabled` → HTTP **403** za sve metode: index/create/store/edit/update/destroy). Legacy controller/model/table/views **zadržani**. Public rollback read preko `CULTURAL_PUBLIC_READ_SOURCE` **zadržan**. Canonical `cultural-event-entries.*` / Moderator / public canonical **neizmijenjeni**. Ovo **nije** hard remove / cutover (Phase B kasnije: uklanjanje flag-a, legacy public read i CRUD koda).
  - **6A residual Package A (`cultural-calendar.day`):** **CLOSED** — **IMPLEMENTED / TESTED / COMMITTED / PUSHED / DEPLOYED / PRODUCTION VERIFIED — EMPTY-DATE SCENARIO CONFIRMED** (`f35cb2e`). Canonical: `CulturalPublicEventQuery::filterByDate()` + `occurrenceOnDate()`; legacy grana / `CULTURAL_PUBLIC_READ_SOURCE` KEEP. Content-bearing day scenario not separately production-smoked (local regression covers; not a blocker). Phase B hard-remove / flag cleanup **OPEN** (nije dio Package A).
...
* Technical Specification — `docs/technical-specifications/Technical-Specification_Javni_portal.md` (TS-009; verzija **1.0.18**; Stable; TD-TS9-01; CR-002 §3.2; CR-003 §3.3; CR-004A §7.1; CR-004B §7.2; Faza 6A; PO-6A11-01; PO-6A09; **PATCH-063** §7.2.4 / §7.3.4–§7.3.6; **PATCH-064** §5.4–§5.5 / §11.3 / TM-JP-23…38; **PO-6B-01…05**; **PO-6B-08/09** §6.7–§6.8; **PO-6B-10** §3.4.1; **PHASE 6B FORMALLY CLOSED** / limited content-smoke; **6A residual day() Package A CLOSED** / PRODUCTION VERIFIED — EMPTY-DATE)
...
* Technical Specification — `docs/technical-specifications/Technical-Specification_Urednicki_portal.md` (TS-010; verzija **1.0.7**; USVOJEN — V1 implementaciono završen za prethodni obuhvat; **PATCH-063 dokumentaciono usvojen i implementiran**; **PATCH-064** = portalna delta u TS-009 (nije TS-010 scope); usklađen sa TS-003 v0.1.10 / TS-004 v0.1.9)
...
| TS-009 | Javni portal | FT-001 | Kalendar kulture | Stable (v1.0.18); Faza 6A + Faza 6B **PRODUCTION ACCEPTED** (limited content-smoke); `day()` Package A **CLOSED** / PRODUCTION VERIFIED — EMPTY-DATE |
