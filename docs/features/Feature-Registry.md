...
FT-001 je aktivan modul sa završenom i produkcijski potvrđenom **Fazom 6A — Javni portal Događaja** (implementirano i testirano). **6A residual Package A** (`cultural-calendar.day` canonical cutover): **IMPLEMENTED / TESTED (local)**; **NOT PRODUCTION VERIFIED**. **Phase B** (legacy hard-remove / flag cleanup) ostaje otvoren. **Faza 6B — Manifestacije:** **FORMALLY CLOSED** — **ADOPTED / DOCUMENTED / IMPLEMENTED / TESTED / DEPLOYED / PRODUCTION ACCEPTED** (WITH LIMITED CONTENT-SMOKE COVERAGE). PO-MF-WF / PO-EV-WF corrective: **PRODUCTION VERIFIED** (`d3c7a96`).
...
- **PO-TS9-08I:** Privremeni feature flag `legacy` XOR `canonical`; zatim uklanjanje flag-a i legacy public/CRUD.
  ...
  - **6A residual Package A (`cultural-calendar.day`):** **IMPLEMENTED / TESTED (local)** — kada je `CULTURAL_PUBLIC_READ_SOURCE=canonical`, `day()` koristi `CulturalPublicEventQuery::filterByDate()` + `occurrenceOnDate()` (isti SSOT kao `indexCanonical` selected-day); legacy grana KEEP. **NOT PRODUCTION VERIFIED.** Phase B hard-remove / flag cleanup **ne** zatvoren.
...
- **TD-TS9-01:** Ruta `cultural-calendar.day` nije dio referentne IA javnog portala; interna tehnička podrška admin toku. **Package A status:** canonical cutover **IMPLEMENTED / TESTED (local)**; legacy rollback KEEP; Phase B OUT.
...
* Technical Specification — `docs/technical-specifications/Technical-Specification_Javni_portal.md` (TS-009; verzija **1.0.17**; Stable; TD-TS9-01; CR-002 §3.2; CR-003 §3.3; CR-004A §7.1; CR-004B §7.2; Faza 6A; PO-6A11-01; PO-6A09; **PATCH-063** §7.2.4 / §7.3.4–§7.3.6; **PATCH-064** §5.4–§5.5 / §11.3 / TM-JP-23…38; **PO-6B-01…05**; **PO-6B-08/09** §6.7–§6.8; **PO-6B-10** §3.4.1; **PHASE 6B FORMALLY CLOSED** / limited content-smoke; **6A residual day() Package A** IMPLEMENTED/TESTED local)
...
| TS-009 | Javni portal | FT-001 | Kalendar kulture | Stable (v1.0.17); Faza 6A + Faza 6B **PRODUCTION ACCEPTED** (limited content-smoke); `day()` Package A IMPLEMENTED/TESTED (local) |
