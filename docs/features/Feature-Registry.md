...
FT-001 je aktivan modul sa završenom i produkcijski potvrđenom **Fazom 6A — Javni portal Događaja** (implementirano i testirano). **6A residual Package A** (`cultural-calendar.day`): **CLOSED** / PRODUCTION VERIFIED — EMPTY-DATE (`f35cb2e` / docs `0ca5096`). **Phase B1+B2** (flag + legacy runtime cleanup): **IMPLEMENTED / TESTED (local)** — canonical = jedini public read; `CULTURAL_PUBLIC_READ_SOURCE` uklonjen; dual-read i legacy CRUD runtime uklonjeni; tabela `cultural_events` KEEP. **B3** (table DROP) = **OPEN / DEFERRED**. **NOT PRODUCTION VERIFIED** (B1+B2). **kk_admin UX / navigation consolidation:** `Kontrolna tabla` + unified `Zahtjevi` + login → KK — **IMPLEMENTED / TESTED / COMMITTED** (`9a72b27`). **PO-ORG/MOD rejected request editor cleanup:** dismiss/soft-hide odbijenih Org/Mod zahtjeva — **ADOPTED / DOCUMENTED / IMPLEMENTED / TESTED (LOCAL)**; **NOT DEPLOYED / NOT PRODUCTION VERIFIED**. **MOD-UX-01 — Moderator UX / navigation corrective:** korisnički entrypoint **Kontrolna tabla**; sadržajni entrypoint **Moderiranje** (Događaji / Manifestacije); `Organizator: <naziv>`; **Promijeni organizatora**; **Izbor organizatora**; terminološka konsolidacija (uklonjeni Radna tabla / Mod rad / Workspace u ovom značenju); auth/lifecycle/`CulturalOrganizerContext` KEEP; bez migracije — **ADOPTED / DOCUMENTED / IMPLEMENTED / TESTED (LOCAL)**; **NOT DEPLOYED / NOT PRODUCTION VERIFIED**. **Faza 6B — Manifestacije:** **FORMALLY CLOSED** — **ADOPTED / DOCUMENTED / IMPLEMENTED / TESTED / DEPLOYED / PRODUCTION ACCEPTED** (WITH LIMITED CONTENT-SMOKE COVERAGE). PO-MF-WF / PO-EV-WF corrective: **PRODUCTION VERIFIED** (`d3c7a96`).
...
- **PO-TS9-08I:** Privremeni feature flag `legacy` XOR `canonical`; zatim uklanjanje flag-a i legacy public/CRUD.
  ...
  - **Phase B1+B2 (CURRENT):** **IMPLEMENTED / TESTED (local)** — flag removed; public portal canonical-only; legacy CRUD routes/controller/views removed; image helpers → `CulturalCalendarDefaultImages`; `CulturalEvent` model class KEEP (non-runtime / B3 shell; table retained). **B3 table DROP = OPEN / DEFERRED.** **NOT PRODUCTION VERIFIED.**
...
- **TD-TS9-01:** Ruta `cultural-calendar.day` nije dio referentne IA javnog portala; interna tehnička podrška admin toku. **Package A status:** canonical cutover **PRODUCTION VERIFIED — EMPTY-DATE SCENARIO CONFIRMED** (`f35cb2e`); legacy rollback KEEP; Phase B OPEN.
...
* Technical Specification — `docs/technical-specifications/Technical-Specification_Javni_portal.md` (TS-009; verzija **1.0.19**; Stable; TD-TS9-01; CR-002 §3.2; CR-003 §3.3; CR-004A §7.1; CR-004B §7.2; Faza 6A; PO-6A11-01; PO-6A09; **PATCH-063** §7.2.4 / §7.3.4–§7.3.6; **PATCH-064** §5.4–§5.5 / §11.3 / TM-JP-23…38; **PO-6B-01…05**; **PO-6B-08/09** §6.7–§6.8; **PO-6B-10** §3.4.1; **PHASE 6B FORMALLY CLOSED** / limited content-smoke; Package A CLOSED; **Phase B1+B2 IMPLEMENTED/TESTED local**; B3 OPEN)
...
* Technical Specification — `docs/technical-specifications/Technical-Specification_Urednicki_portal.md` (TS-010; verzija **1.0.10**; USVOJEN — V1 implementaciono završen za prethodni obuhvat; **PATCH-063 dokumentaciono usvojen i implementiran**; **MOD-UX-01** = UI/navigation status sync; **PATCH-064** = portalna delta u TS-009 (nije TS-010 scope); usklađen sa TS-003 v0.1.10 / TS-004 v0.1.9)
...
| TS-009 | Javni portal | FT-001 | Kalendar kulture | Stable (v1.0.19); Faza 6A + 6B PRODUCTION ACCEPTED; Package A CLOSED; Phase B1+B2 IMPLEMENTED/TESTED (local); B3 OPEN |
| TS-010 | Urednički portal | FT-001 | Kalendar kulture | Usvojen (v1.0.10) — V1 + PATCH-063; MOD-UX-01 UI status sync; PATCH-064 nije TS-010 scope (portal = TS-009) |
