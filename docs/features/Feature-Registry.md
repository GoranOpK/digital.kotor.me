...
**Newsletter (u okviru FT-001):** dobrovoljna pretplata na `User` nalog (nije automatska pri registraciji); jedna pretplata po korisniku; režimi „Svi događaji“ (dinamički) i „Odabrani organizatori“ + „Bez organizatora“; aktivacija bez dodatnog e-mail confirmationa; odjava/reaktivacija nad istim zapisom; preference samo ubuduće; aktivna pretplata ≠ dozvoljena isporuka; nema praznog Newslettera. Model sadržaja i dalje: novoobjavljeni događaji i poslovno značajne promjene (objavljivanje = prvo uključivanje; otkazivanje/odlaganje/promjena datuma/vremena/lokacije = prioritet, samo pretplatnicima kojima je događaj već poslat). Manifestacija nije kriterijum pretplate u V1. Postojeća implementacija je testna — bez migracije testnih pretplatnika. Bez fiksnog sedmičnog poslovnog rasporeda.
