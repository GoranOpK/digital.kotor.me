...
| FT-003     | Evidencija aktivnosti (Kalendar kulture) | Usvojen (TS-012 v1.0.6) — **Faza 8 STARTED / F8-03 emitters PO ACCEPTED (local); V1 best-effort audit; Admin UI pending** | FS §5.16 + PATCH-FS-074; BM-14; V1 katalog uključuje Manifestacije; van opsega: pregled/filteri, retention, izvoz (BR-188); **nema durable audit replay**; `repeatable()` uniqueness = known V1 limitation |
...
FT-001 je aktivan modul sa završenom i produkcijski potvrđenom **Fazom 6A — Javni portal Događaja** — **CLOSED** (canonical-only SSOT; B1+B2 PRODUCTION VERIFIED / CLOSED; categories **14/14 PASS**; dual-read/write = 0; implementation remaining **NONE**). **6A residual Package A** (`cultural-calendar.day`): **CLOSED** / PRODUCTION VERIFIED — EMPTY-DATE (`f35cb2e` / docs `0ca5096`). **Phase B1** (flag/config removal) + **Phase B2** (canonical-only public + legacy CRUD runtime removal): **IMPLEMENTED / TESTED / PRODUCTION VERIFIED / CLOSED**. **B3** (table DROP) = **DEFERRED / NON-RUNTIME / NON-BLOCKING**. **kk_admin UX / navigation consolidation:** `Kontrolna tabla` + unified `Zahtjevi` + login → KK — **IMPLEMENTED / TESTED / COMMITTED** (`9a72b27`). **PO-ORG/MOD rejected request editor cleanup:** dismiss/soft-hide odbijenih Org/Mod zahtjeva — **ADOPTED / DOCUMENTED / IMPLEMENTED / TESTED (LOCAL)**; **NOT DEPLOYED / NOT PRODUCTION VERIFIED**. **MOD-UX-01 — Moderator UX / navigation corrective:** korisnički entrypoint **Kontrolna tabla**; sadržajni entrypoint **Moderiranje** = hub link (`cultural-moderator-workspace.index`; **Događaji organizatora** / **Manifestacije organizatora**); `Organizator: <naziv>`; **Promijeni organizatora**; **Izbor organizatora**; desktop sizing `128×38`; mobile hamburger bez Alpine (inline vanilla JS); terminološka konsolidacija (uklonjeni Radna tabla / Mod rad / Workspace u ovom značenju); auth/lifecycle/`CulturalOrganizerContext` KEEP; bez migracije — **ADOPTED / DOCUMENTED / IMPLEMENTED / TESTED (LOCAL)**; **NOT DEPLOYED / NOT PRODUCTION VERIFIED**. TS-010 → **v1.0.11**. **Faza 6B — Manifestacije:** **FORMALLY CLOSED** — **ADOPTED / DOCUMENTED / IMPLEMENTED / TESTED / DEPLOYED / PRODUCTION ACCEPTED** (WITH LIMITED CONTENT-SMOKE COVERAGE). PO-MF-WF / PO-EV-WF corrective: **PRODUCTION VERIFIED** (`d3c7a96`).

**Newsletter (u okviru FT-001):** **FAZA 7 / TS-011 = FORMALLY CLOSED.** Kanonska pretplata na `User` (nije automatska pri registraciji); jedna pretplata po korisniku; režimi „Svi događaji“ i „Odabrani organizatori“ + „Bez organizatora“; aktivacija bez dodatnog e-mail confirmationa; odjava/reaktivacija nad istim zapisom; preference samo ubuduće; aktivna pretplata ≠ dozvoljena isporuka; nema praznog Newslettera. Regular `cultural-calendar:send-newsletter` (6h) + priority `cultural-calendar:send-newsletter-priority` (5 min). Manifestacija nije kriterijum pretplate u V1. Legacy weekly runtime **disabled**; fizički legacy artefakti KEEP. **Bez** migracije testnih pretplatnika. Settings URI = `/newsletter`. TS-012 emit = **Faza 8**.
...
* Technical Specification — `docs/technical-specifications/Technical-Specification_Javni_portal.md` (TS-009; verzija **1.0.19**; Stable; TD-TS9-01; CR-002 §3.2; CR-003 §3.3; CR-004A §7.1; CR-004B §7.2; Faza 6A; PO-6A11-01; PO-6A09; **PATCH-063** §7.2.4 / §7.3.4–§7.3.6; **PATCH-064** §5.4–§5.5 / §11.3 / TM-JP-23…38; **PO-6B-01…05**; **PO-6B-08/09** §6.7–§6.8; **PO-6B-10** §3.4.1; **PHASE 6B FORMALLY CLOSED** / limited content-smoke; Package A CLOSED; **Phase B1+B2 PRODUCTION VERIFIED / CLOSED**; **FAZA 6A CLOSED**; B3 DEFERRED)
...
* **FAZA 7** = NL-01…NL-06 **IMPLEMENTED / FORMALLY CLOSED** (eligibility + regular/priority delivery + legacy weekly disabled). Settings = `/newsletter`. **TS-012 emit/storage = Faza 8.** Legacy physical files KEEP. Production evidence = **PO-CONFIRMED**; live Git HEAD = **UNOBSERVED** iz Cursora.
...
| TS-012 | Evidencija aktivnosti | FT-003 | Kalendar kulture | Usvojen (v1.0.6) — **Faza 8 STARTED / F8-03 emitters PO ACCEPTED (local); V1 best-effort / no durable replay; Admin UI pending** |
...
Usvojen (TS-012 v1.0.6) — **Faza 8 STARTED / F8-03 emitters PO ACCEPTED (local). V1 audit = best-effort / failure-isolated / no durable replay. `repeatable()` uniqueness = known V1 limitation. Admin UI pending. Nije COMPLETED.**
...
* Technical Specification — `docs/technical-specifications/Technical-Specification_Evidencija_aktivnosti.md` (TS-012; verzija **1.0.6**; USVOJEN; **F8-03 emitters PO ACCEPTED local**; V1 best-effort; Admin UI pending)

Matrica sljedivosti (sažetak):

| BM | FS | FT | TS |
|----|----|----|-----|
| BM-AL-01–BM-AL-08 | BR-170–BR-188, BR-349–BR-350 / §5.16 | FT-003 | TS-012 (usvojen v1.0.6; F8-03 emitters PO ACCEPTED local; V1 best-effort) |
| BM-EP-09 | §5.16 | FT-003 | TS-012 (usvojen v1.0.6; F8-03 emitters PO ACCEPTED local) |
| BM-GL-09, BM-GL-20 | BR-170, BR-174 | FT-003 | TS-012 (usvojen v1.0.6; F8-03 emitters PO ACCEPTED local) |
| BM-MF-20 | §5.16 katalog Manifestacije | FT-003 | TS-012 (usvojen v1.0.6; F8-03 emitters PO ACCEPTED local) |
