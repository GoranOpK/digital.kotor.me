...
{
    /**
     * Run MEGA diagnose via scripts/mega-archive.js diagnose (read-only).
     *
     * @return array{
     *     ok: bool,
     *     email_present: bool,
    ...
    public function run(): array
    {
        ...
        $baseFolder = (string) config('services.mega.base_folder', 'digital.kotor');
        ...
        $classifier = app(MegaArchiveFailureClassifier::class);
        ...
        $timeout = max(1, (int) config('external_archive.diagnose_timeout_seconds', 120));
        ...
        try {
            ...
            return [
                ...
                'folder_found' => false,
                'root_children_sample' => [],
                'error' => $classifier->sanitize('MEGA diagnose process exception: '.$e->getMessage()),
                'node_binary' => $binary,
                'user_agent' => $userAgent,
            ];
        }
        ...
        if ($parsed !== null) {
            ...
            if (($parsed['error'] ?? '') !== '') {
                $parsed['error'] = $classifier->sanitize((string) $parsed['error']);
            }
        ...
        return [
            ...
            'folder_found' => false,
            'root_children_sample' => [],
            'error' => $classifier->sanitize('MEGA diagnose: invalid or empty JSON from script. '.mb_substr($hint, 0, 400)),
            'node_binary' => $binary,
            'user_agent' => $userAgent,
        ];
    }
    ...
    {
        $ua = trim((string) config('services.mega.user_agent', 'DigitalKotorArchive/1.0'));

        return $ua !== '' ? $ua : 'DigitalKotorArchive/1.0';
    }
