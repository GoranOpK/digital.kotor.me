...
/**
 * Resolves Step 5 verify report paths under a non-public base directory.
 */
final class IdentityVerifyProtectedOutputPath
{
    public static function baseDirectory(): string
    {
        return storage_path('app/private/identity-verify');
    }
    ...
    public static function ensureBaseDirectory(): string
    {
        ...
        if (! is_dir($base) && ! mkdir($base, 0770, true) && ! is_dir($base)) {
            throw new InvalidArgumentException('Unable to prepare protected verify output directory.');
        }

        $resolved = realpath($base);
        if ($resolved === false) {
            throw new InvalidArgumentException('Unable to resolve protected verify output directory.');
        }
    ...
    public static function resolve(string $path): string
    {
        $path = trim($path);
        if ($path === '') {
            throw new InvalidArgumentException('Verify output path is required.');
        }
        ...
        if (! self::isInside($resolved, $base)) {
            throw new InvalidArgumentException('Verify output path must be inside the protected verify directory.');
        }

        foreach (self::forbiddenRoots() as $forbidden) {
            if ($forbidden !== null && self::isInside($resolved, $forbidden)) {
                throw new InvalidArgumentException('Verify output path must not be web-accessible.');
            }
        }
