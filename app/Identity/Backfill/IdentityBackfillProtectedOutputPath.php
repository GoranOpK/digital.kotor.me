...
/**
 * Resolves Step 4 backfill report paths under a non-public base directory.
 */
final class IdentityBackfillProtectedOutputPath
{
    public static function baseDirectory(): string
    {
        return storage_path('app/private/identity-backfill');
    }
    ...
    public static function ensureBaseDirectory(): string
    {
        ...
        if (! is_dir($base) && ! mkdir($base, 0770, true) && ! is_dir($base)) {
            throw new InvalidArgumentException('Unable to prepare protected backfill output directory.');
        }

        $resolved = realpath($base);
        if ($resolved === false) {
            throw new InvalidArgumentException('Unable to resolve protected backfill output directory.');
        }
    ...
    public static function resolve(string $path): string
    {
        $path = trim($path);
        if ($path === '') {
            throw new InvalidArgumentException('Backfill output path is required.');
        }
        ...
        if (! self::isInside($resolved, $base)) {
            throw new InvalidArgumentException('Backfill output path must be inside the protected backfill directory.');
        }

        foreach (self::forbiddenRoots() as $forbidden) {
            if ($forbidden !== null && self::isInside($resolved, $forbidden)) {
                throw new InvalidArgumentException('Backfill output path must not be web-accessible.');
            }
        }
