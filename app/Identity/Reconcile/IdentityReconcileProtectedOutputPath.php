...
/**
 * Resolves Step 7 reconcile report paths under a non-public base directory.
 */
final class IdentityReconcileProtectedOutputPath
{
    public static function baseDirectory(): string
    {
        return storage_path('app/private/identity-reconcile');
    }
    ...
    public static function ensureBaseDirectory(): string
    {
        ...
        if (! is_dir($base) && ! mkdir($base, 0770, true) && ! is_dir($base)) {
            throw new InvalidArgumentException('Unable to prepare protected reconcile output directory.');
        }

        $resolved = realpath($base);
        if ($resolved === false) {
            throw new InvalidArgumentException('Unable to resolve protected reconcile output directory.');
        }
    ...
    public static function resolve(string $path): string
    {
        $path = trim($path);
        if ($path === '') {
            throw new InvalidArgumentException('Reconcile output path is required.');
        }
        ...
        if (! self::isInside($resolved, $base)) {
            throw new InvalidArgumentException('Reconcile output path must be inside the protected reconcile directory.');
        }

        foreach (self::forbiddenRoots() as $forbidden) {
            if ($forbidden !== null && self::isInside($resolved, $forbidden)) {
                throw new InvalidArgumentException('Reconcile output path must not be web-accessible.');
            }
        }
