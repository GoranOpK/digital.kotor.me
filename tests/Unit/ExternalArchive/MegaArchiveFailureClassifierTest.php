...
class MegaArchiveFailureClassifierTest extends TestCase
{
    ...
    {
        $this->assertFalse($this->classifier->isTransient('folder not found: digital.kotor'));
    }
    ...
    public function test_sanitize_redacts_password_and_truncates(): void
    {
        $raw = 'Login failed password=SuperSecretMegaPassword-xyz '.str_repeat('x', 600);
        $sanitized = $this->classifier->sanitize($raw);

        $this->assertStringNotContainsString('SuperSecretMegaPassword-xyz', $sanitized);
