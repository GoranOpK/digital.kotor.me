...
<div class="space-y-4">
    <div>
        ...
        <input
            ...
            id="naziv"
            name="naziv"
            value="{{ old('naziv', $location->naziv ?? '') }}"
            required
            maxlength="255"
    ...
    <div>
        ...
        <textarea
            ...
            maxlength="5000"
            class="w-full rounded-md border-gray-300 shadow-sm focus:border-red-700 focus:ring-red-700"
        >{{ old('opis', $location->opis ?? '') }}</textarea>
        @error('opis')
            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
        @enderror
    </div>
    ...
    <div>
        ...
        >
            @foreach($statuses as $status)
                <option value="{{ $status }}" @selected(old('status', $location->status ?? \App\Models\CulturalLocation::STATUS_ACTIVE) === $status)>
