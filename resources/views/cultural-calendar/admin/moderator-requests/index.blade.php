...
<div class="kk-shell mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    @if(!empty($activeStatusFilter))
        <div class="mb-4 rounded-md bg-blue-50 border border-blue-200 text-blue-900 px-4 py-3 text-sm flex flex-wrap gap-3 items-center justify-between">
            <div>Aktivni filter: status = <strong>{{ \App\Models\CulturalModeratorRequest::STATUS_LABELS[$activeStatusFilter] ?? $activeStatusFilter }}</strong></div>
            <a href="{{ route('cultural-moderator-requests.index') }}" class="underline">Ukloni filter</a>
        </div>
    @endif
    @if(session('status'))
