...
<div class="kk-shell mx-auto px-4 sm:px-6 lg:px-8 py-6">
    <div style="display:flex; justify-content:space-between; align-items:center; gap:12px; margin-bottom:16px; flex-wrap:wrap;">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1 mb-0">Status: {{ $entry->statusLabel() }} · ID {{ $entry->id }} · sadržaj je zaključan</p>
        </div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 max-w-3xl mb-6">
        ...
        <dl class="grid grid-cols-1 gap-3 text-sm">
            ...
            <div>
                <dt class="text-gray-500">Naslovna medija</dt>
                <dd class="text-gray-900">{{ $entry->coverMedia?->naziv ?? '—' }}</dd>
            </div>
            <div>
    ...
    <div class="bg-white rounded-lg border border-gray-200 p-6 max-w-3xl">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Lifecycle akcije</h2>
        ...
        <form method="POST" action="{{ route('cultural-event-entries.return', $entry) }}" class="space-y-3">
            @csrf
            <div>
                <label for="return_reason" class="block text-sm font-medium text-gray-700 mb-1">Razlog vraćanja na doradu (obavezno)</label>
                <textarea id="return_reason" name="return_reason" rows="3" required class="w-full rounded-md border-gray-300 shadow-sm">{{ old('return_reason') }}</textarea>
            </div>
