                            ...
                            <select name="document_type" id="document-type-select-{{ $application->id }}" class="form-control" style="font-size: 13px; padding: 8px;" required onchange="updateMergedDocumentNameField({{ $application->id }})">
                                ...
                                @php
                                    ...
                                    if ($omladinskoPackage) {
                                        ...
                                        $order = ['licna_karta', 'crps_resenje', 'pib_resenje', 'pdv_resenje', 'potvrda_neosudjivanost', 'uvjerenje_opstina_porezi', 'potvrda_upc_porezi', 'ioppd_obrazac', 'potvrda_zavod_nezaposleni', 'predracuni_nabavka'];
                                    ...
                                    $documentLabels = [
                                        'potvrda_zavod_nezaposleni' => \App\Models\Application::getZavodNezaposleniDocumentLabel(),
                                        'licna_karta' => ($isDooOstalo && $isRazvoj) ? 'Ovjerenu kopiju lične karte osnivačice ili jedne od osnivačica i izvršne direktorice' : 'Ovjerena kopija lične karte',
                                        ...
                                        'pdv_resenje' => ($isPreduzetnicaFizicko && $isZapocinjanje) ? 'Rješenje o registraciji za PDV (ukoliko ima registrovanu djelatnost i ako je obveznik PDV-a) ili potvrdu da nije PDV obveznik' : (($isPreduzetnicaFizicko && $isRazvoj) ? 'Rješenje o registraciji za PDV (ako je obveznik PDV-a) ili potvrdu da nije PDV obveznik (ukoliko nije PDV obveznik)' : 'Rješenje o registraciji za PDV' . ($isRazvoj ? ' (ako je obveznik PDV-a)' : '')),
                                    ...
                                    if ($application->applicant_type === 'preduzetnica' && $isRazvoj) {
                                        $documentLabels['potvrda_neosudjivanost'] = 'Potvrda da se ne vodi krivični postupak na ime preduzetnice izdatu od Osnovnog suda';
                                        ...
                                        $documentLabels['potvrda_upc_porezi'] = 'Potvrda Poreske uprave o urednom izmirivanju poreza i doprinosa ne stariju od 30 dana, na ime preduzetnice';
                                        $documentLabels['ioppd_obrazac'] = 'Odgovarajući obrazac ovjeren od strane Poreske uprave za poslijednji mjesec uplate poreza i doprinosa za zaposlene, kao dokaz o broju zaposlenih (IOPPD Obrazac) ili potvrdu ovjerenu od strane Poreske uprave da preduzetnica nema zaposlenih';
                                        $documentLabels['dokaz_ziro_racun'] = 'Dokaz o broju poslovnog žiro računa preduzetnice';
                                    } elseif ($isPreduzetnicaFizicko) {
                                        $documentLabels['potvrda_neosudjivanost'] = 'Potvrda da se ne vodi krivični postupak na ime podnositeljke prijave odnosno preduzetnice izdatu od Osnovnog suda';
                                        ...
                                        $documentLabels['potvrda_upc_porezi'] = ($isRazvoj) ? 'Potvrda Poreske uprave o urednom izmirivanju poreza i doprinosa ne stariju od 30 dana na ime preduzetnika' : 'Potvrda Uprave za javne prihode o urednom izmirivanju poreza';
                                        $documentLabels['ioppd_obrazac'] = ($isRazvoj) ? 'Odgovarajući obrazac za posljednji mjesec uplate poreza i doprinosa za zaposlene, kao dokaz o broju zaposlenih (IOPPD Obrazac) ili potvrdu ovjerenu od Poreske uprave' : 'Obrazac IOPPD';
                                        $documentLabels['dokaz_ziro_racun'] = ($isZapocinjanje) ? 'Dokaz o broju poslovnog žiro računa preduzetnice (ukoliko ima registrovanu djelatnost)' : 'Dokaz o broju poslovnog žiro računa preduzetnice';
                                    } elseif ($isDooOstalo && $isZapocinjanje) {
                                        $documentLabels['potvrda_neosudjivanost'] = 'Potvrda da se ne vodi krivični postupak na ime podnositeljke prijave odnosno na ime osnivačice ili jedne od osnivačica i izvršne direktorice izdatu od strane Osnovnog suda';
                                        ...
                                        $documentLabels['potvrda_neosudjivanost'] = 'Potvrda da se ne vodi krivični postupak na ime društva i na ime osnivačice ili jedne od osnivačica i izvršne direktorice izdatu od strane Osnovnog suda';
                                        ...
                                        $documentLabels['potvrda_upc_porezi'] = 'Potvrdu Poreske uprave o urednom izmirivanju poreza i doprinosa ne stariju od 30 dana, na ime osnivačice ili jedne od osnivačica i izvršne direktorice i na ime društva';
                                        ...
                                        $documentLabels['uvjerenje_opstina_nepokretnost'] = 'Uvjerenje Opštine o nepostojanju nepokretnosti';
                                        $documentLabels['potvrda_upc_porezi'] = 'Potvrda Uprave za javne prihode o urednom izmirivanju poreza';
                                        $documentLabels['ioppd_obrazac'] = 'Obrazac IOPPD';
                                    }
        ...
        <div class="info-card">
            ...
            @php
                ...
                if ($omladinskoPackage) {
                    ...
                    $order = ['licna_karta', 'crps_resenje', 'pib_resenje', 'pdv_resenje', 'statut', 'karton_potpisa', 'potvrda_neosudjivanost', 'uvjerenje_opstina_porezi', 'uvjerenje_opstina_nepokretnost', 'dokaz_ziro_racun', 'potvrda_zavod_nezaposleni', 'predracuni_nabavka'];
                    foreach ($requiredDocs as $docType) {
                        if (!in_array($docType, $order)) {
                            $order[] = $docType;
                        }
                ...
                $isPreduzetnica = in_array($application->applicant_type, ['preduzetnica', 'fizicko_lice']);
                    ...
                    $documentLabels['licna_karta'] = ($isDooOstalo && $isRazvoj) ? 'Ovjerenu kopiju lične karte osnivačice ili jedne od osnivačica i izvršne direktorice' : (($isDooOstalo && $isZapocinjanje) ? 'Ovjerenu kopiju lične karte' : 'Ovjerena kopija lične karte');
