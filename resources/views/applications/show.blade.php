                            ...
                            <select name="document_type" class="form-control" style="font-size: 13px; padding: 8px;" required>
                                ...
                                @php
                                    ...
                                    if ($application->applicant_type === 'preduzetnica' && $application->business_stage === 'započinjanje') {
                                        ...
                                        $order = ['licna_karta', 'crps_resenje', 'pib_resenje', 'pdv_resenje', 'potvrda_neosudjivanost', 'uvjerenje_opstina_porezi', 'potvrda_upc_porezi', 'ioppd_obrazac'];
                                    ...
                                    $documentLabels = [
                                        ...
                                        'karton_potpisa' => 'Karton potpisa',
                                        'potvrda_neosudjivanost' => ($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') ? 'Potvrda da se ne vodi krivični postupak na ime preduzetnika izdatu od strane Osnovnog suda' : 'Potvrda o neosuđivanosti',
                                        'uvjerenje_opstina_porezi' => (($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && ($application->business_stage === 'započinjanje' || $application->business_stage === 'razvoj')) ? 'Uvjerenje od organa lokalne uprave o izmirenim obavezama po osnovu lokalnih javnih prihoda na ime preduzetnika ne starije od mjesec dana' : 'Uvjerenje Opštine o urednom izmirivanju poreza',
                                        'uvjerenje_opstina_nepokretnost' => ($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage !== 'razvoj' ? 'Uvjerenje od organa lokalne uprave o urednom izmirivanju poreza na nepokretnost na ime preduzetnice' : 'Uvjerenje Opštine o nepostojanju nepokretnosti',
                                        'potvrda_upc_porezi' => ($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage === 'razvoj' ? 'Potvrda Poreske uprave o urednom izmirivanju poreza i doprinosa ne stariju od 30 dana na ime preduzetnika' : 'Potvrda Uprave za javne prihode o urednom izmirivanju poreza',
                                        'ioppd_obrazac' => ($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage === 'razvoj' ? 'Odgovarajući obrazac za poslijednji mjesec uplate poreza i doprinosa za zaposlene, kao dokaz o broju zaposlenih (IOPPD Obrazac) ili potvrdu ovjerenu od Poreske uprave' : 'Obrazac IOPPD',
                                        'godisnji_racuni' => 'Godišnji računi',
        ...
        <div class="info-card">
            ...
            @php
                ...
                if ($application->applicant_type === 'preduzetnica' && $application->business_stage === 'započinjanje') {
                    ...
                    $order = ['licna_karta', 'crps_resenje', 'pib_resenje', 'pdv_resenje', 'statut', 'karton_potpisa', 'potvrda_neosudjivanost', 'uvjerenje_opstina_porezi', 'uvjerenje_opstina_nepokretnost', 'biznis_plan_usb'];
                    foreach ($requiredDocs as $docType) {
                        if (!in_array($docType, $order)) {
                            $order[] = $docType;
                        }
                    ...
                    $order = ['licna_karta', 'crps_resenje', 'pib_resenje', 'pdv_resenje', 'statut', 'karton_potpisa', 'godisnji_racuni', 'potvrda_neosudjivanost', 'uvjerenje_opstina_porezi', 'uvjerenje_opstina_nepokretnost', 'potvrda_upc_porezi', 'ioppd_obrazac', 'biznis_plan_usb'];
                    foreach ($requiredDocs as $docType) {
                        if (!in_array($docType, $order)) {
                            $order[] = $docType;
                        }
                ...
                $documentLabels = [
                    ...
                    'crps_resenje' => 'Rješenje o upisu u CRPS' . (($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage === 'započinjanje' ? ' (ukoliko ima registrovanu djelatnost)' : (($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') && $application->business_stage === 'započinjanje' ? ' (ukoliko ima registrovanu djelatnost)' : '')),
                    'pib_resenje' => (($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage === 'započinjanje' ? 'Rješenje o PIB-u PJ Poreske uprave (ukoliko ima registrovanu djelatnost)' : 'Rješenje o registraciji PJ Uprave prihoda i carina' . ((($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') && $application->business_stage === 'započinjanje') ? ' (ukoliko ima registrovanu djelatnost)' : '')),
                    'pdv_resenje' => (($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage === 'započinjanje' ? 'Rješenje o registraciji za PDV (ukoliko ima registrovanu djelatnost i ako je obveznik PDV-a) ili potvrdu da nije PDV obveznik (ukoliko nije PDV obveznik)' : (($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage === 'razvoj' ? 'Rješenje o registraciji za PDV (ako je obveznik PDV-a) ili potvrdu da nije PDV obveznik (ukoliko nije PDV obveznik)' : 'Rješenje o registraciji za PDV' . ((($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') && $application->business_stage === 'započinjanje') ? ' (ukoliko ima registrovanu djelatnost i ako je obveznik PDV-a)' : ($application->business_stage === 'razvoj' ? ' (ako je obveznik PDV-a)' : '')))),
                    ...
                    'potvrda_neosudjivanost' => ($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') ? 'Potvrda da se ne vodi krivični postupak na ime preduzetnika izdatu od strane Osnovnog suda' : (($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') && $application->business_stage === 'razvoj' ? 'Potvrda o neosuđivanosti za krivična djela na ime nosioca biznisa (osnivačice ili jedne od osnivača i izvršne direktorice) izdatu od strane Osnovnog suda' : (($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') ? 'Potvrda o neosuđivanosti za krivična djela na ime preduzetnice izdatu od strane Osnovnog suda' : 'Potvrda o neosuđivanosti')),
                    'uvjerenje_opstina_porezi' => (($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && ($application->business_stage === 'započinjanje' || $application->business_stage === 'razvoj')) ? 'Uvjerenje od organa lokalne uprave o izmirenim obavezama po osnovu lokalnih javnih prihoda na ime preduzetnika ne starije od mjesec dana' : (($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') && $application->business_stage === 'razvoj' ? 'Uvjerenje od organa lokalne uprave o urednom izmirivanju poreza na ime nosioca biznisa (osnivačice ili jedne od osnivača i izvršne direktorice) i na ime društva po osnovu prireza porezu, članskog doprinosa, lokalnih komunalnih taksi i naknada' : 'Uvjerenje od organa lokalne uprave o urednom izmirivanju poreza na ime preduzetnice po osnovu prireza porezu, članskog doprinosa, lokalnih komunalnih taksi i naknada'),
                    'uvjerenje_opstina_nepokretnost' => ($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') ? 'Uvjerenje od organa lokalne uprave o urednom izmirivanju poreza na nepokretnost na ime preduzetnice' : (($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') && $application->business_stage === 'razvoj' ? 'Uvjerenje od organa lokalne uprave o urednom izmirivanju poreza na nepokretnost na ime nosioca biznisa (osnivačice ili jedne od osnivača i izvršne direktorice) i na ime društva' : 'Uvjerenje od organa lokalne uprave o urednom izmirivanju poreza na nepokretnost na ime preduzetnice'),
                    'potvrda_upc_porezi' => ($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage === 'razvoj' ? 'Potvrda Poreske uprave o urednom izmirivanju poreza i doprinosa ne stariju od 30 dana na ime preduzetnika' : (($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') && $application->business_stage === 'razvoj' ? 'Potvrda Uprave prihoda i carina o urednom izmirivanju poreza i doprinosa ne stariju od 30 dana, na ime nosioca biznisa (osnivačice ili jedne od osnivača i izvršne direktorice) i na ime društva' : 'Potvrda Uprave za javne prihode o urednom izmirivanju poreza'),
                    'ioppd_obrazac' => ($application->applicant_type === 'preduzetnica' || $application->applicant_type === 'fizicko_lice') && $application->business_stage === 'razvoj' ? 'Odgovarajući obrazac za poslijednji mjesec uplate poreza i doprinosa za zaposlene, kao dokaz o broju zaposlenih (IOPPD Obrazac) ili potvrdu ovjerenu od Poreske uprave' : (($application->applicant_type === 'doo' || $application->applicant_type === 'ostalo') && $application->business_stage === 'razvoj' ? 'Odgovarajući obrazac za poslijednji mjesec uplate poreza i doprinosa za zaposlene ovjeren od Uprave prihoda i carina, kao dokaz o broju zaposlenih (IOPPD Obrazac)' : 'Obrazac IOPPD'),
