...
<div class="container mx-auto px-4 py-8">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <h1 class="text-3xl font-bold mt-2">Dostupnost vrste — {{ $type->name }}</h1>
            <p class="text-sm text-gray-600 mt-1">Fail-closed: bez aktivnog pravila vrsta nije dostupna. Nije stvarno 17/41 mapiranje. Nije production-ready.</p>
        </div>
        <a href="{{ route('admin.e-payments.payment-types.availabilities.create', $type) }}" class="bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700">Novo pravilo</a>
    </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($rules as $rule)
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm">
                            @if($rule->is_active)
                                <form method="POST" action="{{ route('admin.e-payments.payment-types.availabilities.deactivate', [$type, $rule]) }}" class="inline" onsubmit="return confirm('Deaktivirati pravilo? Red se ne briše.');">
                                    @csrf
                                    <button type="submit" class="text-red-600 hover:text-red-800">Deaktiviraj</button>
                                </form>
                            @else
                                <form method="POST" action="{{ route('admin.e-payments.payment-types.availabilities.activate', [$type, $rule]) }}" class="inline">
                                    @csrf
                ...
                @empty
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-center text-gray-500">Nema pravila dostupnosti. Korisnik ne vidi ovu vrstu dok se ne doda aktivno pravilo.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
