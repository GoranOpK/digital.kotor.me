...
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <h1 class="text-3xl font-bold mb-2">Izmijeni vrstu plaćanja</h1>
    <p class="text-sm text-gray-600 mb-6">Interni kod se ne može mijenjati.</p>
    ...
    <form method="POST" action="{{ route('admin.e-payments.payment-types.update', $type) }}" class="bg-white rounded-lg shadow p-6 space-y-4">
        @csrf
        @method('PUT')
        <div>
            <label class="block text-sm font-medium mb-1">Interni kod</label>
            <input type="text" value="{{ $type->code }}" class="w-full border rounded px-3 py-2 font-mono bg-gray-100" disabled>
        </div>
        <div>
            <label class="block text-sm font-medium mb-1">Naziv</label>
            <input type="text" name="name" value="{{ old('name', $type->name) }}" class="w-full border rounded px-3 py-2" required>
        </div>
        <div>
