...
<div class="session-page">
    <div class="session-card">
        <h1>Evidencija druge sjednice</h1>
        <p style="color:#4b5563; font-size:14px; margin-bottom:20px;">
            {{ $competition->title }} — Platforma ne predlaže niti zakazuje termin. Predsjednik unosi vrijeme održavanja i prisustvo sva 3 člana.
        </p>

        @if($firstSession?->held_at)
            <p style="color:#1e3a8a; font-size:14px; margin-bottom:16px;">
                Rok od 7 dana od prve sjednice:
                <strong>{{ $secondDeadlineAt?->format('d.m.Y H:i') }}</strong>
                @if($secondDeadlineOverdue)
                    — rok je prekoračen; prekoračenje ne sprečava ručni unos i ne kreira automatsku odluku.
                @endif
            </p>
        @endif
        ...
        @if($readonly)
            <p style="font-weight:600; color:#065f46;">Druga sjednica je potvrđena i ne može se mijenjati.</p>
        ...
        @else
            <form method="POST" action="{{ $session ? route('commission-sessions.second.update', $competition) : route('commission-sessions.second.store', $competition) }}">
                @csrf
                @if($session)
                    @method('PUT')
                @endif
                ...
                <div class="form-group">
                    <label class="form-label">Prisutni članovi (sva 3)</label>
            ...
            @if($session)
                <form method="POST" action="{{ route('commission-sessions.second.confirm', $competition) }}" style="margin-top:16px;">
                    @csrf
        ...
        <h2 style="font-size:18px; font-weight:700; color:#0B3D91; margin:32px 0 12px;">Usmeno predstavljanje prijava</h2>
        @if($eligibleApplications->isEmpty())
            <p style="color:#6b7280;">Nema prijava koje trenutno ispunjavaju uslove za usmeno predstavljanje.</p>
        @else
            <ul>
                @foreach($eligibleApplications as $eligibleApplication)
                    @php $oral = $eligibleApplication->oralPresentation; @endphp
                    <li style="margin-bottom:8px;">
                        {{ $eligibleApplication->business_plan_name }}
                        —
                        @if($oral?->isCompleted())
                            završeno
                        @elseif($oral)
                        ...
                        @else
                            nije evidentirano
                        @endif
                        <a href="{{ route('commission-sessions.oral.edit', [$competition, $eligibleApplication]) }}">Evidencija usmenog</a>
                    </li>
                @endforeach
            </ul>
        @endif
